                       ...
                       if plotIt==1 && i==trial_nb
                            ...
                            %calculate CI for threshold from marginalized posterior distribution
                             ...
                             disp(['Threshold confidence interval (95%) = [',sprintf('%2.f"',thr_CI(1)),'-',sprintf('%2.f"',thr_CI(2)),']'])
                                ...
                                text(1,max(marg_thr)/2,sprintf('MAP: %2.f"',curr_est_max_thr))
                                text(1,max(marg_thr)/3,['p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,14))])
                                text(1,max(marg_thr)/4,['CI_9_5% = [',sprintf('%2.f"',thr_CI(1)),'-',sprintf('%2.f"',thr_CI(2)),']'])
                                ...
                                disp(['MAP threshold: ',sprintf('%2.f"',curr_est_max_thr),' / p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,14))])
                                
                                subplot(2,3,2)
                                maxPS = max(psi.new_slopes); minPS = min(psi.new_slopes);
                                plot(psi.new_slopes,marg_pslo,'r'); hold on
                                text(maxPS/3,max(marg_pslo)/2,sprintf('MAP: %.2f',curr_est_max_pos_slo))
                                text(maxPS/3,max(marg_pslo)/3,['p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,15))])
                                ...
                                ylabel('Marginalized probability')
                                disp(['Positive slope: ',sprintf('MAP: %.2f',curr_est_max_pos_slo),' / p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,15))])
                                
                                subplot(2,3,3)
                                maxNS = max(psi.new_neg_slopes); minNS = min(psi.new_neg_slopes);
                                plot(psi.new_neg_slopes,marg_nslo,'r'); hold on
                                text(maxNS/3,max(marg_nslo)/2,sprintf('MAP: %.2f',curr_est_max_neg_slo))
                                text(maxNS/3,max(marg_nslo)/3,['p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,16))])
                                ...
                                ylabel('Marginalized probability')
                                disp(['Negative slope: ',sprintf('MAP: %.2f',curr_est_max_neg_slo),' / p(MAP+-',num2str(round(tolerance.*100)),'%) = ',sprintf('%2.1f%%',history(end,16))])
                                ...
                                ylabel('% CR')
                                text(1, 0.35, sprintf('MAP: %d"',round(curr_est_max_thr)));
                                text(1, 0.25,  sprintf('p(stereoblind) = %2.1f%%',100*sum(marg_thr((10.^disparities)>=1300))))
